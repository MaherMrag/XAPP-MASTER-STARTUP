...
module.exports = function (RED) {
  ...
  RED.nodes.registerType("s7.db", function MongoConfigNode(n) {
  ...
  function getClient(config) {
    ...
    if (!poolCell) {
      mongoPool['#' + config.deploymentId] = poolCell = {
        ...
        "promise": mongodb.MongoClient.connect(config.uri, /* config.options ||  */{ useUnifiedTopology: true, useNewUrlParser: true , useCreateIndex: true,useMongoClient: true}).then(function (client) {
  ...
  RED.nodes.registerType("s7.db.in", function MongoInputNode(n) {
    RED.nodes.createNode(this, n);
    ...
    getClient(node.config).then(function (client) {
      ...
      function handleMessage(msg, cb) {
        ...
        // console.log('operation updateOne :',operation === operations['findOneAndUpdate'])
        ...
        //if (operation === operations['insertMany']) console.log('Array.isArray(args) : ',Array.isArray(args))
        ...
        //(operation === operations['updateOne']) //Modif : Mongodb/Collection.js : Collection.prototype.updateOne = function(filter, update, options, callback) 
          ...
          operation.apply(collection || client.db, args.concat(function (err, response) {
            ...
            if (forEachEnd != err) {
              ...
              if (forEachIteration == err) {
                ...
                //node.emit('response',messageToSend); //Modif
                // if (cb) cb(messageToSend); //Modif
                node.send(messageToSend);
              } else {
                // No need to clone - the same message will not be sent again.
                msg.payload = response;

                //node.emit('response',msg); //Modif
                if (cb) cb(msg); //Modif
                node.send(msg);
              }
            }
